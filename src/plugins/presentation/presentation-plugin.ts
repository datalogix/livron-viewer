...
export class PresentationPlugin extends Plugin {
  ...
  protected init() {
    this.on('presentationrequest', () => this.request())
  }
  ...
  async request() {
    ...
    if (
      ...
      console.warn('Ignoring Spread modes when entering Presentation, since the document may contain varying page sizes.')
  ...
  private notifyStateChange(state: PresentationModeState) {
    this.state = state
    this.dispatch('presentationmodechanged', { state })
  }
