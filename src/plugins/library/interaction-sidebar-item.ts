...
export class InteractionSidebarItem extends SidebarItem {
  ...
  build() {
    const container = createElement('div', 'interactions')
  ...
  protected renderList(container: HTMLElement) {
    ...
    const progressBar = new ProgressBar(
      ...
      (this.interactionService?.pending.length || 0) + (this.interactionService?.inProgress.length || 0),
    )
    ...
    this.interactionService?.allGroup().forEach((interactions) => {
      ...
      interactions.forEach((interaction) => {
        const button = createElement('button', ['interaction', `interaction-type-${interaction.type}`, `interaction-status-${interaction.status}`], { type: 'button' })
        ...
        button.appendChild(createElement('span', 'interaction-status'))
