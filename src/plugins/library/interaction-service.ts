...
export class InteractionService extends Dispatcher {
  protected interactions: Interaction[] = []
  protected interactionsPerPage = new Map<number, Interaction[]>()
  ...
  load(interactions: Interaction[] = []) {
    ...
    interactions.forEach((interaction) => {
      const interactionsPerPage = this.interactionsPerPage.get(interaction.page) ?? []
      interactionsPerPage.push(interaction)
      this.interactionsPerPage.set(interaction.page, interactionsPerPage)
    })

    this.dispatch('interactionloaded', { interactions: this.interactions })
  }
  ...
  destroy() {
    this.interactions = []
    this.interactionsPerPage.clear()
  }
