...
export class FindPlugin extends Plugin {
  ...
  protected init() {
    this.on('find', params => this.onFind(params))
    this.on('findbarclose', () => this.onFindBarClose())
    this.on('documentinit', () => {
      // requestIdleCallback(() => this.extractText())
    })
    this.on('onepagerendered', () => this.firstPageCapability?.resolve())
  ...
  scrollMatchIntoView({
    ...
    scrollIntoView(element, {
      top: -50, // MATCH_SCROLL_OFFSET_TOP
      left: selectedLeft + -400, // MATCH_SCROLL_OFFSET_LEFT
    }, /* scrollMatches = */ true)
  }
    ...
    switch (state.type) {
      case 'again': {
        ...
        return pageNumber >= 1
          ...
          && !this.viewer.getVisiblePages().ids.has(pageNumber)
      }
